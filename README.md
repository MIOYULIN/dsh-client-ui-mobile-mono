...
DeepSeek Harness（DSH）Web UI 的**移动端适配插件**：手机浏览器自动切换抽屉式布局，并为所有端注入**黑白（单色）主题**。桌面端零影响。
...
```sh
git clone https://github.com/MIOYULIN/dsh-m-ui.git
cd dsh-m-ui
...
pnpm dsh web --patch /绝对路径/dsh-m-ui/cordis.patch.yml
```
