# ui-v2 — dsh-client-ui-mobile-mono v0.4.15
...
- **会话统计**：官方统计条移动端隐藏，收进 composer 下折叠胶囊；点开为双列统计卡（轮次/LLM/工具/TTFT/速度/缓存/tokens/上下文占用），尾部带 `MONO · v*` 版本脚注；展开上滑入场、收起下滑退出（可中断回放）；卡片无阴影（防点击重绘闪烁）
...
```
ui-v2/
├── package.json   # @local/dsh-client-ui-mobile-mono @ 0.4.15
├── install.sh
├── uninstall.sh
├── cordis.patch.yml
├── demo/
