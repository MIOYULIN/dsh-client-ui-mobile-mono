# ui-v2 — dsh-client-ui-mobile-mono v0.4.18

DSH Web 移动端插件的浏览器端实现：抽屉布局 + 黑白主题 + 偏好设置卡 + 折叠统计 + Session log 适配。

## 特性

- **移动端布局**（UA 检测，仅手机浏览器启用）：会话列独占全宽；侧栏 → 左侧覆盖抽屉（可选 320/360/400 三档宽度，展开态零投影——景深由遮罩承担）；详情列 → 右侧全宽抽屉（JS 列标记 `data-dshmu-*` + hash 无关 `[class*=detailsCol]` 双通道，点会话统计条即可展开）；左缘右滑开抽屉、拖拽实时跟手、速度判定开合
...
```
ui-v2/
├── package.json   # @local/dsh-client-ui-mobile-mono @ 0.4.18
├── install.sh
├── uninstall.sh
├── cordis.patch.yml
├── demo/
