# ui-v2 — dsh-client-ui-mobile-mono v0.4.20

DSH Web 移动端插件的浏览器端实现：抽屉布局 + 黑白主题 + 偏好设置卡 + 折叠统计 + Session log 适配。

## 特性

- **移动端布局**（UA 检测，仅手机浏览器启用）：会话列独占全宽；侧栏 → 左侧覆盖抽屉（可选 320/360/400 三档宽度，展开态零投影）；详情列 → 右侧全宽抽屉（JS 列标记 `data-dshmu-*` + hash 无关 `[class*=detailsCol]` 双通道，点会话统计条即可展开）；左缘右滑开抽屉、拖拽实时跟手、速度判定开合；遮罩完全透明（仅点击关闭，无黑色变暗，景深由中列收缩表达）
- **设置弹窗**：全屏 sheet + 底部标签栏（拇指可达，刘海屏安全区自适应）；**切标签动画**——内容节块整体上滑 + 轻右移入场（子块级联），激活标签顶部指示条弹性展开；弹窗内所有可交互元素统一动效语言——按压回缩 + 松手回弹、状态换色过渡（官方元素按 role/标签通配覆盖）；内容头大标题上滑入场、关闭钮弹性浮现；「移动端界面」偏好卡：黑白主题、抽屉宽度、左缘手势、隐藏模型名，`localStorage` 持久化并热应用；卡片浮升入场 + 徽标旋入，分段选择器三等分滑块弹性滑动 + 选中轻弹确认，开关拇指按压延展
- **模型选择器**：隐藏模型名、只显示思考等级（可在设置卡关闭）
- **会话统计**：官方统计条移动端隐藏，收进 composer 下折叠胶囊；点开为双列统计卡（轮次/LLM/工具/TTFT/速度/缓存/tokens/上下文占用），尾部带 `MONO · v*` 版本脚注；展开/收起为高度过渡（`grid-template-rows` 0fr↔1fr，上方内容无跳变）+ 内容上滑/淡出（可中断回放）
...
```
ui-v2/
├── package.json   # @local/dsh-client-ui-mobile-mono @ 0.4.20
├── install.sh
├── uninstall.sh
├── cordis.patch.yml
├── demo/
