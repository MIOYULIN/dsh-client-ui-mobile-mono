# ui-v2 — dsh-client-ui-mobile-mono v0.4.19
...
- **设置弹窗**：全屏 sheet + 底部标签栏（拇指可达，刘海屏安全区自适应）；弹窗内所有可交互元素统一动效语言——按压回缩 + 松手回弹、状态换色过渡（官方元素按 role/标签通配覆盖）；内容头大标题上滑入场、关闭钮弹性浮现；「移动端界面」偏好卡：黑白主题、抽屉宽度、左缘手势、隐藏模型名，`localStorage` 持久化并热应用；卡片浮升入场 + 徽标旋入，分段选择器三等分滑块弹性滑动 + 选中轻弹确认，开关拇指按压延展
...
```
ui-v2/
├── package.json   # @local/dsh-client-ui-mobile-mono @ 0.4.19
├── install.sh
├── uninstall.sh
├── cordis.patch.yml
├── demo/
