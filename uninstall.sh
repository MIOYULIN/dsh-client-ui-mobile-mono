#!/usr/bin/env bash
# dsh-client-ui-mobile-mono 卸载脚本
...
echo "== dsh-client-ui-mobile-mono uninstaller =="
...
if [ -f "$PATCH" ]; then
  # 移除本插件的组合块（注释行 + 两行插入块）
  sed -i '/# Mobile drawer layout + monochrome theme (@local\/dsh-client-ui-mobile-mono)/,/^$/{d}' "$PATCH"
  sed -i '/- id: ui-mobile-mono/{N;/name: .@local\/dsh-client-ui-mobile-mono./d}' "$PATCH"
